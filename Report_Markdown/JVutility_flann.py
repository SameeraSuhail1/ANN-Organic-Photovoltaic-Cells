...
def plot_true_vs_pred(ytest, yhat, ytrain, ytrain_out, norm_lims, str_path, exp_str, j,which_conc_feature, optimizer = ""):
    ...
    plt.savefig(fname=str_path + "/"+ exp_str + "/figures/test_learned_nh_" + str(j) + which_conc_feature + optimizer + ".pdf")
    plt.show()
    ...
    plt.ylabel('J(mA/cm^2)', fontsize=20)
    plt.suptitle('J Prediction of Training Samples',fontsize=20)
    plt.savefig(fname=str_path + "/"+ exp_str + "/figures/train_learned_nh_" + str(j) + which_conc_feature + optimizer + ".pdf")
    plt.show()
    #e.g. power/dataout/exp1_2/figures/trained_learned_flann
    #str_path + "/"+ exp_str +"/"+  r'FLANN_performances_1_2.csv
...
#    print("Size of test data: ",len(dftest))
 #Put all the features in a list (required for the next for loop where target_name is chosen)
    ...
    for j in temp_list:
        ...
        elif j[0:1] == 'V' and len(j)>1:   #for flann exp1.1 where features are feature13_4, wavelength1, wavelength2 etc.
            continue
        else:
            target_name= 'target'
