...
features = ["feature0","feature1","feature30"]
for m in features:
    concentration= m[7:]
    
    df_normed_feature = pd.DataFrame(df_normed.loc[:,["target"+concentration,m]], columns=["target"+ concentration,m])
    
    which_target = 'target' + m[7:]
    df_normed_feature = df_normed_feature[np.isfinite(df_normed_feature[m])]
    df_normed_feature = df_normed_feature[np.isfinite(df_normed_feature[which_target])]
    ...
    def which_flann(df, flann_polynomial):
        if flann_polynomial=='chebyshev':
            ...
            df['V2']=1
            df['V3']=2*df[m]**2-1
            df['V4']=2*df[m]**3-3*df[m]
            df['V5']=8*df[m]**4-8*df[m]**2+1
            
        elif flann_polynomial=='legendre':
            str_path="legendre"
            df['V2']=1
            df['V3']=(3*df[m]**2-1)/2
            df['V4']=(5*df[m]**3-3*df[m])/2
            df['V5']=(35*df[m]**4-30*df[m]**2+3)/8
        elif flann_polynomial=='trigonometric':
            str_path="trigonometric"
            df['V2']=np.cos(df[m])
            df['V3']=np.sin(df[m])
            df['V4']=np.cos(2*math.pi*df[m])
            df['V5']=np.sin(2*math.pi*df[m])
    ...
    print(df_normed_feature)




    feature_list=[m ,'V2','V3','V4','V5']
    ...
    for j in range(1):
        ...
        import pickle 
        pklfile = open(file=str_path + "/"+ exp_str + "/" + m + 'FLANN_JV_1_1.pkl', mode="wb")
...
#str_path="power"
pklfile = open(file=str_path + "/"+ exp_str + "/" + m + 'FLANN_JV_1_1.pkl', mode="rb")
