...
features=['feature0','feature1','feature2_5','feature6','feature13_4','feature30']
for m in features:
    num_hidden_neuron= 8
    #make a model with specified hyper parameters
    xdftrain, ydftrain,xdftest, ydftest, xtrain, ytrain, xtest, ytest,  = split_tt(df_normed,70,102, m)
    ...
    for i in range(n_ensemble-1):
        xdftrain, ydftrain,xdftest, ydftest, xtrain, ytrain, xtest, ytest,  = split_tt(df_normed,70,i+1,m)
        #-------------seed------------------
        seed(i+1)
        set_random_seed(i+1)
        #-----------------------------------
    ...
    temp = pd.DataFrame({'features': m, 'Overall dB': overall_MSEdB, 'Test dB': test_MSEdB, 'Train dB': train_MSEdB,
    ...
    import pickle 
#    saved_data= open(exp_str + "/" + 'test_transmit_1_1.pkl','wb')    
...
#    saved_data.close()
    ...
    pklfile = open(file=exp_str + "/train_learned" + str(num_hidden_neuron) + m + ".pkl", mode="wb")
...
#    plot_true_vs_pred(ytest_unnormed, yhat_unnormed, ytrain_unnormed, ytrain_out_unnormed, exp_str, j)
    plot_true_vs_pred(ytest,yhat,ytrain,ytrain_out, norm_lims,exp_str,8, m,"")
    


export_csv = performance_out.to_csv(exp_str +"/"+ r'Ynew_performances_1_1_other.csv', index = None, header=True) #Don't forget to add '.csv' at the end of the path

pklfile = open(file=exp_str + "/train_learned" + str(num_hidden_neuron) + m+".pkl", mode="rb")
