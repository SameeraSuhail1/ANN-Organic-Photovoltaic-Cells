...
xdftrain, ydftrain, xdftest, ydftest, xtrain, ytrain, xtest, ytest = split_tt(df_normed,70,102,'wavelength','concentration')
...
for j in range(3,20):
    ...
    for i in range(n_ensemble-1):
        xdftrain, ydftrain,xdftest, ydftest, xtrain, ytrain, xtest, ytest,  = split_tt(df_normed,70,i+1,'wavelength','concentration')
        #-------------seed------------------
        seed(i+1)
        set_random_seed(i+1)
        #-----------------------------------
    ...
    import pickle 
#    saved_data= open('test_transmit_1_2.pkl','wb')    
...
#    saved_data.close()
    ...
    pklfile = open(file=exp_str + "/train_learned_nh_" + str(num_hidden_neuron) + ".pkl", mode="wb")
...
export_csv = performance_out.to_csv(exp_str +"/"+ r'Ynew_performances_1_2.csv', index = None, header=True) #Don't forget to add '.csv' at the end of the path


pklfile = open(file=exp_str + "/train_learned_nh_" + str(num_hidden_neuron) + ".pkl", mode="rb")
