...
features = ["feature0","feature1","feature2_5","feature6","feature30"]

for m in features:
    concentration= m[7:]

    df_normed_feature = pd.DataFrame(df_normed.loc[:,["target"+concentration,m]], columns=["target"+ concentration,m])
   # df_normed=df_normed.loc[:, df_normed.columns.str.endswith(concentration)]
    ...
    def which_flann(df, flann_polynomial):
        if flann_polynomial=='chebyshev':
            str_path="chebyshev"
            df['wavelength2']=1
            df['wavelength3']=2*df[m]**2-1
            df['wavelength4']=2*df[m]**3-3*df[m]
            df['wavelength5']=8*df[m]**4-8*df[m]**2+1
        elif flann_polynomial=='legendre':
            str_path="legendre"
            df['wavelength2']=1
            df['wavelength3']=(3*df[m]**2-1)/2
            df['wavelength4']=(5*df[m]**3-3*df[m])/2
            df['wavelength5']=(35*df[m]**4-30*df[m]**2+3)/8
            
            
        elif flann_polynomial=='trigonometric':
            str_path="trigonometric"
            df['wavelength2']=np.cos(df[m])
            df['wavelength3']=np.sin(df[m])
            df['wavelength4']=np.cos(2*math.pi*df[m])
            df['wavelength5']=np.sin(2*math.pi*df[m])
            
        elif flann_polynomial=='power':
            str_path="power"
            df['wavelength2']=1
            df['wavelength3']=df[m]**2
            df['wavelength4']=df[m]**3
            df['wavelength5']=df[m]**4
    ...
    str_path, df_normed_expanded = which_flann(df_normed_feature, 'chebyshev')
    #e.g. power/dataout/exp1_1
    os.makedirs(str_path + "/"+ exp_str +"/figures", exist_ok=True)
    print(df_normed_expanded)




    feature_list=[m ,'wavelength2','wavelength3','wavelength4','wavelength5']
...
#Check the numpy arrays shape and dimension
    ...
    for j in range(1):
        ...
        pklfile = open(file=str_path + "/"+ exp_str + "/" + m + 'FLANN_transmit_1_1.pkl', mode="wb")
        toPickle = {'xtrain': xtrain, 'ytrain':ytrain , 'xtest':xtest , 'ytest':ytest , 'yhat':yhat, 'ytrain_out':ytrain_out, 'entire_predicted':entire_predicted , 'entire_true': entire_true
        ...
        export_csv = performance_out.to_csv(str_path + "/"+ exp_str +"/"+ m + r'FLANN_performances_1_1.csv', index = None, header=True) #Don't forget to add '.csv' at the end of the path
        ...
        plot_true_vs_pred(ytest,yhat,ytrain,ytrain_out, norm_lims,str_path, exp_str,j,m, "")
...
#str_path="power"
pklfile = open(file=str_path + "/"+ exp_str + "/" + m + 'FLANN_transmit_1_1.pkl', mode="rb")
